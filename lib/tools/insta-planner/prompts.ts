...
각 시간대(또는 슬라이드)는 \`[라벨]\` 한 줄로 시작.
시간대 안의 모든 라인은 **반드시 아래 prefix 중 하나로 시작**:

- 🎙️ "..."  ← 출연자가 입으로 말하는 대사 (큰따옴표 필수)
- 🎨 ...    ← 화면 연출 (텍스트 오버레이, 시각 효과, B-roll, 동작 디렉션 등)

prefix 없는 라인 금지. 마크다운 헤더(#, **) 금지. 빈 줄은 OK.
...
[0:00-0:03 🎬 후킹]
🎙️ "이거 하나면 콘텐츠 만드는 시간 10배 줄어요"
🎨 텍스트 오버레이: "콘텐츠 시간 1/10 ⏱️" (크고 굵게, 펀치 인)
🎨 시각: 일식당 분위기 필터 + 로고 등장, 손가락으로 화면 터치

[0:03-0:15 📱 본론 1]
🎙️ "벤치마킹 영상 분석부터 반자동 편집까지 한 플랫폼에서 다 됩니다"
🎨 텍스트 오버레이: "벤치마킹 분석 ✓ / 반자동 편집 ✓ / 유튜브 세팅 ✓ / 쇼츠 자동생성 ✓"
🎨 시각: 각 기능 아이콘이 오마카세 접시처럼 하나씩 등장
...
🎨 ...
`
    : `**${idea.format === "carousel" ? "캐러셀" : "피드"} 예시 형식**:

[Slide 1 후킹]
🎙️ (캡션에서 1줄 대사처럼 읽힐 텍스트)
🎨 텍스트: 큰 후킹 문구
🎨 시각: ...
...
본문만 작성. 위 예시는 형식 참고용일 뿐, 내용은 새로 만들 것. 메타 설명, 출력 외 머리말 금지.`;
}
